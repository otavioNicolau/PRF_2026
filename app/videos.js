  ...
  return (
    <SafeAreaView style={styles.containerArea}>
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        <Text style={[styles.title]}>{parsedVideo.titulo.toUpperCase()}:</Text>
        ...
        <View>
          ...
          <View style={styles.row}>
            {Object.keys(parsedVideo.resolucoes).map(resolution => (
              <TouchableOpacity
                key={resolution}
                style={[
                  styles.resolutionButton,
                  { backgroundColor: resolution === selectedResolution ? '#666' : '#333' }
                ]}
                onPress={() => handleResolutionChange(resolution)}
              >
                <Text style={styles.resolutionButtonText}>{resolution}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <TouchableOpacity onPress={openVideoInBrowser} style={styles.downloadButton}>
            <MaterialIcons name="play-circle-filled" size={24} color="white" />
            <Text style={styles.downloadButtonText}>Assistir no Navegador</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
